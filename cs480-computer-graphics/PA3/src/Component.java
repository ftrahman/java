...
/**
 * The implementation for an object that can draw itself (rotation, direction, children).
 * 
 * Boiler Plate:
...
public class Component implements Rotatable, Nameable, Colorable, UpdatingDisplayable {
	...
	public boolean beeX = false;
	public boolean beeY = false;
	public boolean beeZ = false;
	public boolean birdX = false;
	public boolean birdY = false;
	public boolean birdZ = false;
	...
	// Overloaded function to handle the change in direction once tank boundary limit is reach
	// flips the direction of the object
	public void rotate(float[] axis, final double angleDelta) {
      ...
      if(this.birdX == true && this.birdX == false) {
          this.yAngle += 180; this.birdX = false;
      }

      else if(this.birdY == true && this.birdX == false) {
          this.xAngle += 180; this.birdY = false;
      }
      //Change in X and Y direction for narwhal 
      else if(this.birdX == true && this.birdX == true){
          this.zAngle += 180; this.birdX = false; this.birdY = false;
      }
      else if(this.beeX == true && this.beeY == false) {
          this.yAngle += 180; this.beeX = false;
      }
      else if(this.beeX == false && this.beeY == true) {
          this.xAngle += 180; this.beeY = false;
      }
      else if(this.beeX == true && this.beeY == true ) {
          this.zAngle += 180; this.beeX = false; this.beeY = false;
      }
  }
