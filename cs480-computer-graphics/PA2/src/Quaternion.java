...
public class Quaternion {
	...
	public Quaternion multiply(final Quaternion that) {
		float new_s = this.s * that.s;
		float dotProduct = (this.v[0] * that.v[0]) + (this.v[1]*that.v[1]) + (this.v[2]*that.v[2]);
		float sa_bx = (this.s * that.v[0]); 
		float sa_by = (this.s * that.v[1]);
		float sa_bz = (this.s * that.v[2]);
		float sb_ax = (that.s * this.v[0]);
		float sb_ay = (that.s * this.v[1]);
		float sb_az = (that.s * this.v[2]);
		float crossProduct = ((this.v[1] * that.v[2]) - (that.v[1] * this.v[2])) +
		                     ((this.v[2] * that.v[0]) - (that.v[2] * this.v[0])) +
		                     ((this.v[0] * that.v[1]) - (that.v[0] * this.v[1]));
		that.s = new_s - dotProduct;
		that.v[0] = sa_bx + sb_ax + crossProduct;
		that.v[1] = sa_by + sb_ay + crossProduct;
		that.v[2] = sa_bz + sb_az + crossProduct;
		return that;
	}
	 ...
	 */
	private float norm() {
		float squares = (float) (Math.pow(this.s, 2) + Math.pow(this.v[0], 2) + Math.pow(this.v[1], 2) + Math.pow(this.v[2], 2));
		float mag = (float) Math.sqrt(squares);
		return mag;
	}
	...
	public void normalize() {
		float mag = norm();
		this.s = this.s / mag;
		this.v[0] = this.v[0] / mag;
		this.v[1] = this.v[1] / mag;
		this.v[2] = this.v[2] / mag;
		return;
	}
