...
public class Component implements Rotatable, Nameable, UpdatingDisplayable, Colorable {
	...
	private FloatColor color = FloatColor.AUBURN;
	...
	public String toString() {
		return "Component[ " + this.name + ", " + this.xAngle + ", " + this.yAngle + ", " + this.zAngle + ", " + this.position + "]";
	}
	...
	/**
	 * 
	 * Handles the rotation for individual joints, including the case
	 * for mirroring joints.
	 * 
	 * {@inheritDoc}
	 * 
	...
	public void rotate(final Axis axis, final double angleDelta) {
		if (axis.equals(Axis.X)) {
		  ...
		  if(this.yPositiveExtent < this.yNegativeExtent) {
			this.yAngle -= angleDelta;
			this.yAngle = Math.max(this.yAngle, this.yPositiveExtent);
			this.yAngle = Math.min(this.yAngle, this.yNegativeExtent);
		  } 
		  else {
		    this.yAngle += angleDelta;
		  ...
		  if(this.zPositiveExtent < this.zNegativeExtent) {
			this.zAngle -= angleDelta;
			this.zAngle = Math.max(this.zAngle, this.zPositiveExtent);
			this.zAngle = Math.min(this.zAngle, this.zNegativeExtent);
		}
		  else {
            this.zAngle += angleDelta;
	...
	public void update(final GL2 gl) {
		...
		gl.glPushMatrix();
		// Translate object to specified position 
		gl.glTranslated(this.position.x(), this.position.y(), this.position.z());
		// Rotate the object given specified angles
		...
		// Draw every component of the ant.
		for (final Component child : this.children) {
		  child.draw(gl);
		  }
